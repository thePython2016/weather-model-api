...
# ── JWT Validation — reads Bearer token from Authorization header ──
...
async def verifyTokenEndpoint(email: str = Depends(verifyToken)):
    ...
    fname = record["fname"] if record else "User"   # ← fixed: was record[0]
...
async def authenticateUser(cred: UserAuthentication):
    ...
    # )
    response.set_cookie(
    key="token",          # ← was "refresh_token"
    value=token,          # ← use JWT token, not refresh_token
    httponly=True,
    samesite="lax",
